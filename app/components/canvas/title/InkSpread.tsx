...
// Text 컴포넌트 임포트 추가
import { Group, Circle, Ring, Image as KonvaImage, Text } from "react-konva";
...
const InkSpread: React.FC<ExtendedProps> = React.memo(
  ({
    ...
    const captionRef = useRef<import("konva/lib/shapes/Text").Text>(null); // Text 레퍼런스 추가
    ...
    const playHoverIn = useCallback(() => {
      ...
      // captionRef도 애니메이션 킬 목록에 추가
      gsap.killTweensOf([mainGroupRef.current, imageRef.current, captionRef.current]);
      if (filterTweenRef.current) filterTweenRef.current.kill();

      gsap.to(mainGroupRef.current, {
        scaleX: 1.15, scaleY: 1.15, duration: 0.25, ease: "power2.out",
      });
      ...
      if (captionRef.current) {
        gsap.to(captionRef.current, {
          opacity: 1, y: size + 25, duration: 0.25, ease: "power2.out",
        });
      }
    ...
    const playHoverOut = useCallback(() => {
      ...
      gsap.to(mainGroupRef.current, {
        scaleX: 1, scaleY: 1, duration: 0.35, ease: "power3.out",
      });
      gsap.to(imageRef.current, {
      ...
      if (captionRef.current) {
        gsap.to(captionRef.current, {
          opacity: 0, y: size + 15, duration: 0.35, ease: "power3.out",
        });
      }
    ...
    useEffect(() => {
      ...
      else if (!isDraggingActive && isMouseOverRef.current && isReadyRef.current) playHoverIn();
    ...
    useEffect(() => {
      ...
      // 캡션 초기 세팅
      if (captionRef.current) {
        tl.set(captionRef.current, { opacity: 0, y: size + 15 });
      }
    ...
    return (
      <Group x={x} y={y}>
        <Group ref={mainGroupRef} scaleX={0} scaleY={0} opacity={0} listening={false}>
          <Group clipFunc={(ctx) => ctx.arc(0, 0, CLIP_RADIUS, 0, Math.PI * 2)} listening={false}>
            <KonvaImage
              ...
              x={0} y={0} width={IMG_SIZE} height={IMG_SIZE} offsetX={CLIP_RADIUS} offsetY={CLIP_RADIUS}
              opacity={0} listening={false}
            />
          </Group>
          <Ring
            ref={inkSpreadRef}
            x={0} y={0} innerRadius={0} outerRadius={size}
            ...
            fillRadialGradientColorStops={[ 0, color, 0.75, color, 1, `rgba(${r},${g},${b},0)` ]}
            listening={false}
          />
        </Group>

        {/* 새로 추가된 캡션 (크기 변형 방지를 위해 mainGroupRef 외부에 배치) */}
        {caption && (
          <Text
            ref={captionRef}
            text={caption}
            x={0} y={size + 15} // 노드 아래쪽 초기 위치
            offsetX={100} // 중앙 정렬을 위한 offset (width의 절반)
            width={200}
            align="center"
            fill="#333333"
            fontSize={16}
        ...
        <Circle
          x={0} y={0} radius={size + 35} fill="rgba(0,0,0,0)"
          onMouseEnter={() => {
