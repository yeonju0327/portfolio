...
* **3D 유리 카드 깨짐(Shatter) 효과:** 플레이어 또는 딜러가 버스트(Bust, 21점 초과)되는 순간, 해당 버스트를 유발한 마지막 유리 카드가 쨍그랑하는 파쇄음(Web Audio API 오실레이터 실시간 합성)과 함께 8개의 큼직한 3D 유리 조각(2x2 격자 구조)으로 산산조각 나며 테이블 아래로 흩뿌려지는 동적 깨짐 물리 연출을 구현했습니다. 특히 깨진 파편 조각들도 원래 카드의 구조를 반영하기 위해 **개별 파편 조각을 하나의 독립적인 미니 3D 카드 그룹으로 취급**했습니다. 몸체에는 원래 카드의 투명하고 맑은 유리 질감(metalness: 0.02, roughness: 0.06, transmission: 0.85, ior: 1.52, clearcoat: 1.0)을 가진 ExtrudeGeometry를 적용하고, 뒷면 이미지는 렌더링하지 않도록 생략하는 대신 그 위에 앞면 데칼(ShapeGeometry + faceTex)만 단일하게 결합하고 `DoubleSide` 속성을 설정하여 조각이 뒤집힐 때도 투명 유리 너머로 앞면 무늬가 자연스럽게 투영되도록 연출했습니다. 또한 겹쳤을 때 파란 틴트가 강하게 뭉치는 현상을 해결하기 위해 유리 파편의 색상을 순수한 백색(`#ffffff`)으로 지정하여 극도로 투명하고 사실적인 유리 질감을 완성했습니다. 파편이 튕겨 낙하할 때는 테이블 위에 남아있는 다른 정상 카드들과 물리적으로 겹쳐 렌더링되는 시각적 간섭을 막기 위해, 정상 카드가 배치되는 Z축 영역(플레이어 구역 Z: 1.0~2.2, 딜러 구역 Z: -2.2~-1.0)을 우회하도록 안착 Z 좌표를 범위 밖으로 튕겨내는 경계 보정 규칙을 추가했습니다. 특히 플레이어 버스트 시에는 **딜러의 숨겨진 카드를 먼저 완전히 오픈한 후 플레이어의 카드가 깨지도록 시퀀스를 설계**하여 시각적 흐름의 완성도를 확보했으며, 비산된 파편은 라운드가 끝날 때까지 테이블 바닥에 그대로 남아있어 영롱한 유리 조각의 아날로그 흔적을 보존하고 새로운 라운드가 시작될 때 씬에서 완전히 제거되고 dispose되도록 최적화했습니다.
